...
static const char *get_capability(struct client *client)
{
	...
	uint32_t flags = IMAP_CAP_VISIBILITY_PREAUTH;
	...
	if (is_login_cmd_disabled(client))
		flags |= IMAP_CAP_VISIBILITY_NO_LOGIN;
...
static void imap_client_create(struct client *client, void **other_sets)
{
	...
	imap_capability_list_add(imap_client->capability_list,
			"LOGINDISABLED", IMAP_CAP_VISIBILITY_REQUIRE_ALL |
							 IMAP_CAP_VISIBILITY_PREAUTH |
	...
	if (!explicit_capability) {
		if (imap_client->set->imap_literal_minus) {
			imap_capability_list_add(imap_client->capability_list,
					"LITERAL-", IMAP_CAP_VISIBILITY_ALWAYS);
		} else {
			imap_capability_list_add(imap_client->capability_list,
					"LITERAL+", IMAP_CAP_VISIBILITY_ALWAYS);
		}
